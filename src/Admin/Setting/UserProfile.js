...
const UserProfile = () => {
  ...
  console.log(PatchEditUserPermissionData);
  ...
  console.log("delel", DeleteAdminSettingDeleteUserData);
  ...
  const EditUserSavebtnFun = (e) => {
    ...
    todate ==null ? toast.warn("Please add Duration's ") : todate 
    && fromdate ==null? toast.warn("Please add Duration's ") :fromdate  && 
    dispatch(PatchEditUserPermission(edit)) && setUserEdit((o) => !o);

    
    console.log("mnbjdbvjhs", edit);
  };
  ...
  const UserEditFun = (e, data) => {
    ...
    // setToDate(data?.permission[0]?.to_date)
    // setFromDate(data?.permission[0]?.from_date)
    setUserEdit((o) => !o);
  ...
  //   dispatch(PostKYCdetail(payload));
  // };
  console.log("eeeee", PostKYCdetailData);
  ...
  useEffect(() => {
    ...
    console.log("jmgvvdcb", oneditfixedpermissiondata, editremovedpermissiondata)

    let arrayData = []
    oneditfixedpermissiondata?.map((item, id) => { //this map has perission on select id

      editremovedpermissiondata?.map((items, ids) => {  //this map have deleted permission on select box

        if (item == items.permission) {
          console.log("itemitemitem!2", item)
      ...
      <div className="dashboard-part  ">
        ...
        <div className="content-sec userprofile-sec">
          ...
          <ul className="userprofile-list">
            {PermissionData()?.VIEW_B2B_USER_PROFILE ==
            "VIEW_B2B_USER_PROFILE"
